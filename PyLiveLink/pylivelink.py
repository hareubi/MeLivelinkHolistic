...
class PyLiveLink:
    ...
    other PyLiveLink compatible programs like this library.
    """

    def __init__(self, name: str = "Python_LiveLink", 
        ...
        self._blend_shapes = [0.000] * BlendShapeNum
        self._old_blend_shapes = []                 # used for filtering
        for i in range(BlendShapeNum):
    ...
    def encode(self) -> bytes:
        ...
        # Define the 'Head' bone attributes
        head_bone["Name"] = "Head"
        head_bone["Parent"] = -1
        head_bone["Location"] = [0, 0, 0]

        # Simulating `real_rotation` values (replace these with actual data)
        real_rotation = {"xyzw": {"x": 0.0, "y": 0.0, "z": 0.0, "w": 1.0}}

        # Use safe access with default values
        head_bone["Rotation"] = [
            real_rotation.get("xyzw", {}).get("x", 0),
            real_rotation.get("xyzw", {}).get("y", 0),
            real_rotation.get("xyzw", {}).get("z", 0),
            real_rotation.get("xyzw", {}).get("w", 1)
        ]

        head_bone["Scale"] = [0, 0, 0]

        # Add the bone to the array
        bone_array.append(head_bone)

        # Populate the result
        json_result["Bone"] = bone_array
        json_result["Parameter"] = param_array

        # Create the final JSON structure
        json_final["android"] = json_result

        # Convert to JSON string and then to bytes
        s = json.dumps(json_final).encode()
        return s
